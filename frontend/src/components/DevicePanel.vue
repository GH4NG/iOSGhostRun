<template>
  <Card class="flex flex-col bg-card border-border shadow-sm">
    ...
    <AlertDialog v-model:open="showDeviceLocked">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>设备被锁定</AlertDialogTitle>
          <AlertDialogDescription>设备当前处于锁定状态，无法挂载开发者镜像。请在设备上解锁后重试。</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>取消</AlertDialogCancel>
          <AlertDialogAction @click="retrySelect" class="bg-primary text-white">已解锁，重试</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
const showDeviceLocked = ref(false)
const lastSelectedDevice = ref<ServiceDeviceInfo | null>(null)
...
async function selectDevice(device: ServiceDeviceInfo) {
  lastSelectedDevice.value = device
  try {
    ...
    try {
      ...
      console.error('CheckDeveloperImage failed:', e)
    ...
    if (
      (msg && msg.includes && (msg.includes('DeviceLocked') || msg.includes('Device Locked') || msg.includes('Device locked'))) ||
      (full && full.includes && full.includes('DeviceLocked')
      )
    ) {
      showDeviceLocked.value = true
    } else {
...
async function retrySelect() {
  showDeviceLocked.value = false
  if (lastSelectedDevice.value) {
    // 重试选择设备并挂载镜像
    await selectDevice(lastSelectedDevice.value)
  }
}
